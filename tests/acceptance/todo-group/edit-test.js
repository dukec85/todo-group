...
test('visiting /todo-groups/1/edit shows a for to edit a todo-group with id 1', function(assert) {
  ...
  andThen(function() {
    assert.equal(currentRouteName(), 'todo-groups.edit',
      'The url /todo-groups/1/edit loads the "todo-groups.edit" route.');
    assert.equal(currentURL(), '/todo-groups/1/edit',
      'The "todo-groups.edit" route does not redirect without user interaction');
...
test('user can submit the form on /todo-groups/1/edit to update todo-group with id 1', function(assert) {
  ...
  andThen(function() {
    assert.equal(currentRouteName(), 'todo-groups.index',
      'After submit redirect to the "todo-groups.index" route.');
