...
test('visiting /todo-groups/1 shows a for to detail a todo-group with id 1', function(assert) {
  ...
  andThen(function() {
    assert.equal(currentRouteName(), 'todo-groups.detail',
      'The url /todo-groups/1 loads the "todo-groups.detail" route.');
    assert.equal(currentURL(), '/todo-groups/1',
      'The "todo-groups.detail" route does not redirect without user interaction');
...
test('user can see done value of todo-items on the todo-groups.detail page', function(assert) {
  server.create('todo-group');
